...
const option = {
    xAxis: {
        type: 'category',
        data: ['the last third month', 'the last second month', 'the last month', 'current month'] // 这里假设你已经知道这四个月的名称
    },
    yAxis: {
        type: 'value',
        min: 0,
...
function updateChart(obj) {
    ...
    $.ajax({
        url:  path + "/jsp/order",
        type: 'GET',
        data: {method:"monthReact"},
        dataType: 'json',
        success: function(response) {
            // 假设后端返回的数据格式是一个包含四个整数的数组
            if (Array.isArray(response) && response.length === 4) {
                // 更新x轴和y轴的数据
                option.xAxis.data =  ['the last third month', 'the last second month', 'the last month', 'current month'];
